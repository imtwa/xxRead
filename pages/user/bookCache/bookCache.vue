<template>
	<view>
		...
		<view v-if="!visnobook" class="page">
			<view class="atop">
				共有 {{bookShelf.length}} 本书，共占用内存 {{currentSize}} MB
			</view>
			<view class="container" v-for="(book,i) in bookShelf" :key="book.imgurl">
				<view class="left-section" @click="handleClearCache(i)">
					...
					<view class="middle">
						<view class="left">
							{{book.count}}/{{book.chapters.length}}
						</view>
						<view class="right">
							<view v-if="book.vis">
								<text>下载中</text>
					...
					<view class="bottom">
						<u-line-progress :percentage="((book.count*100)/book.chapters.length).toFixed(2)"
							activeColor="#00A8FF">
						</u-line-progress>
					</view>
				</view>
			...
			<!-- 确认清除全部缓存弹窗 -->
			<u-modal :show="vismodalH" :title="title" showCancelButton closeOnClickOverlay :zoom="false"
		...
		computed: {
			bookShelf() {
				...
				for (let i = 0; i < books.length; i++) {
					...
					books[i].chapters.filter(chapter => chapter.visD === true).forEach(chapter => {
						count++;
					});
					books[i].count = count
				}
