  ...
  <view style="height: 100vh">
    ...
    <view
      ...
      :style="{
        ...
        boxShadow:
          showShadow && turnType === 0 ? '0 0 10px 0 rgba(0,0,0,.4)' : '',
      }"
    ...
    >
      ...
      >
        ...
        <view class="head">
          ...
          <u-icon
            ...
            @click="back"
          ></u-icon>
          <view style="margin-bottom: 12px; font-size: 14px; color: #eee">{{
            book.bookname
          }}</view>
        </view>
      </view>
      ...
      <view
        class="setting"
        :style="{ bottom: settingShow ? 0 : `-100%` }"
        @touchend.stop
      >
        <view class="item">
          <view class="item-name">字号</view>
          <view class="icon" @click="bigSize" v-if="fontSize < maxFontSize"
            >A+</view
          >
          <view class="icon" style="color: #666; border: #666 solid 1px" v-else
            >A+</view
          >
          <view class="icon" @click="smallSize" v-if="fontSize > minFontSize"
            >A-</view
          >
          <view class="icon" style="color: #666; border: #666 solid 1px" v-else
            >A-</view
          >
          <view class="icon" @click="changeFont(2)" v-if="simplified === 1"
            >繁體</view
          >
          <view
            class="icon"
...
export default {
  ...
  methods: {
    ...
    async initPage() {
      ...
      if (
        this.history.chapterIndex > this.directoryList.length - 1 ||
        !this.history.chapterIndex
      ) {
      ...
      let page = Math.floor(
        (this.curChapter.totalPage - 1) * this.history.progress,
      );
      this.goToPage(page);
      uni.hideLoading();
    },
    ...
    calcHeight() {
      if (this.contentHeight) {
        ...
        let lineNum = Math.floor(
          (this.contentHeight + Math.floor((lineHeight - this.fontSize) / 2)) /
            lineHeight,
        );
          ...
          this.$nextTick(() => {
            ...
            query
              ...
              .boundingClientRect((data) => {
                ...
                // #ifdef APP-PLUS || MP-WEIXIN
                lineHeight =
                  Math.floor(lineHeight * this.pixelRatio) / this.pixelRatio;
                // #endif
                let lineNum = Math.floor(
        ...
        setTimeout(() => {
          ...
          query
            ...
            .boundingClientRect((data) => {
              ...
              // #endif
              this.curChapter.totalPage =
                Math.ceil(height / this.innerHeight) || 1;
      ...
      if (type === "next") {
        ...
        if (this.curChapter.chapterIndex !== this.directoryList.length - 1) {
          ...
          this.nextChapter = {
            ...
            chapterName:
              this.directoryList[this.curChapter.chapterIndex + 1].name,
          };
          await this.getOneChapter(
            this.directoryList[this.curChapter.chapterIndex + 1].chapterId,
          );
    ...
    async getOneChapter(chapterId, showLoading) {
      ...
      if (
        this.bookall.chapters[chapindex].hasOwnProperty("text") &&
        !this.refreshData
      ) {
        // 对象包含 text 属性
        //直接用
