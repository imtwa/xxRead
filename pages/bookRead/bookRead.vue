...
<template>
	<view style="height: 100vh;" :class="currentFontFamily">
		...
		<view class="container hidden">
			...
			<view id="content" class="content">
				<view class="inner-box" :style="{ height: `${innerHeight}px` }">
					<view class="book-inner" id="preChapter" v-html="preChapter.text" :style="{
		...
		<view class="container hidden">
			...
			<view class="content">
				<view class="inner-box" :style="{ height: `${innerHeight}px` }">
					<view class="book-inner" id="curChapter" v-html="curChapter.text" :style="{
              fontSize: `${fontSize}px`,
              lineHeight: `${lineHeight * fontSize}px`
            }">
					</view>
		...
		<view class="container hidden">
			...
			<view class="content">
				<view class="inner-box" :style="{ height: `${innerHeight}px` }">
					<view class="book-inner" id="nextChapter" v-html="nextChapter.text" :style="{
              fontSize: `${fontSize}px`,
              lineHeight: `${lineHeight * fontSize}px`
            }">
					</view>
		...
		<!-- ************************** -->
		<!-- 封面 -->
		<view class="cover container" :class="{
        ...
        boxShadow: showShadow && turnType === 0 ? '0 0 10px 0 rgba(0,0,0,.4)' : ''
			...
			<view class="content">
				...
				<view class="inner-box" :style="{ height: `${innerHeight}px` }" v-if="prePage.canRead">
					...
					<view class="book-inner" v-html="prePage.text" :style="{
              fontSize: `${fontSize}px`,
              lineHeight: `${lineHeight * fontSize}px`,
          ...
          directoryList[chapterProgress].name
			...
			<view class="setting" :style="{ bottom: settingShow ? 0 : `-100%` }" @touchend.stop>
				<view class="item">
					...
					<view class="icon" @click="changeFont(1)" v-else style="border: #ff9900 solid 1px; color: #ff9900">
						繁體
					</view>
				</view>
				<view class="item">
					...
					<view style="width: 80%">
						<u-scroll-list style="padding-top: 30rpx;padding-bottom: 0;" indicatorActiveColor="#ff9900">
							<view class="font-face fontFamily0" @click="FontFamily('fontFamily0')"
								:class="{ active: currentFontFamily === 'fontFamily0' }" style="min-width: 180rpx;">默认</view>
								
							<view class="font-face fontFamily1" @click="FontFamily('fontFamily1')"
								:class="{ active: currentFontFamily === 'fontFamily1' }">方正新楷体</view>
								
							<view class="font-face fontFamily2" @click="FontFamily('fontFamily2')"
								:class="{ active: currentFontFamily === 'fontFamily2' }">仓耳今楷04-W03</view>
								
							<view class="font-face fontFamily3" @click="FontFamily('fontFamily3')"
								:class="{ active: currentFontFamily === 'fontFamily3' }">仓耳今楷05-W05</view>
						</u-scroll-list>
					</view>
				</view>
				<view class="item">
			...
			<!-- 目录层 -->
			<view class="directory" :class="{
          container0: background === 1,
          container1: background === 2,
          container2: background === 3
        }" v-if="directoryShowBefore" :style="{
						...
						this.$nextTick(() => {
							...
							query
								...
								.boundingClientRect(data => {
									...
									// #endif
									let lineNum = Math.floor(
										(height + Math.floor((lineHeight - this.fontSize) / 2)) /
										lineHeight
									)
									this.innerHeight = lineNum * lineHeight
									resolve()
