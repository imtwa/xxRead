...
const getSearch = {
    ...
    try {
      ...
      if (trList.length > 1) {
        for (let i = 1; i < trList.length; i++) {
          ...
          const imgurl = `http://www.paoshu8.info/files/article/image/${imgMatch[1]}/${imgMatch[2]}/${imgMatch[2]}s.jpg`
  ...
  search2: async keyword => {
    ...
    try {
      ...
      const doc = new HTMLParser(res.data.toString().trim())
      const ui = new HTMLParser(doc.getElementsByClassName('txt-list-row5')[0]['innerHTML'])
      const trList = ui.getElementsByTagName('li')
      let book = []
      // 该网站下标从1开始
      for (let i = 1; i < trList.length; i++) {
      ...
      while (true) {
        ...
        if (res.data.booklist) {
          ...
          for (let i = 0; i < search.length; i++) {
            ...
            let imgurl =
              'http://wfqqreader-1252317822.image.myqcloud.com/cover/' +
              n +
              '/' +
              s +
              '/b_' +
            ...
            if (search[i].totalWords) {
              if (search[i].totalWords > 10000) {
                let word = (search[i].totalWords / 10000).toFixed(2)
                tags.push(word + '万字')
              } else {
    ...
    try {
      ...
      if (trList.length > 0) {
        for (let i = 0; i < trList.length; i++) {
          ...
          // 获取链接和书名
          const bookurl = origin + trdoc.getElementsByTagName('a')[0].attributes.href
  ...
  search7: async keyword => {
    ...
    try {
      ...
      const doc = new HTMLParser(res.data.toString().trim())
      const ui = new HTMLParser(doc.getElementsByClassName('txt-list')[0]['innerHTML'])
      const trList = ui.getElementsByTagName('li')
  ...
  search9: async keyword => {
    ...
    try {
      ...
      const doc = new HTMLParser(res.data.toString().trim())
      const ui = new HTMLParser(doc.getElementsByClassName('panel-body')[0]['innerHTML'])
  ...
  search10: async keyword => {
    ...
    let url = 'http://www.hljcxzj.com/so/keyword/'.replace('keyword', keyword)
    ...
    try {
      ...
      const doc = new HTMLParser(res.data.toString().trim())
      const ui = new HTMLParser(doc.getElementsByClassName('ul_m_list')[0]['innerHTML'])
      const trList = ui.getElementsByTagName('li')
      let book = []
      for (let i = 0; i < trList.length; i++) {
        const trdoc = new HTMLParser(trList[i]['innerHTML'])
        ...
        const bookname = spant1.attributes.title.replace(/\s/g, '')
        // 获取作者
        const spant2 = trdoc.getElementsByTagName('a')[3].attributes.title.replace(/\s/g, '')
        const author = spant2

        let tags = []
        // 分类 字数 状态
        tags.push(
          new HTMLParser(trdoc.getElementsByClassName('c')[0].innerHTML).getElementsByTagName(
            'a'
          )[0].attributes.title
        )
        tags.push(trdoc.getElementsByClassName('words')[0].innerHTML + '字')
        tags.push(
          new HTMLParser(trdoc.getElementsByClassName('abover')[0].innerHTML).getElementsByTagName(
            'font'
          )[0].innerHTML
        )
  ...
  search11: async keyword => {
    ...
    let url = 'https://www.maodouzw.com/search3?searchkey=keyword&searchtype=all'.replace(
      'keyword',
      keyword
    )
    ...
    try {
      ...
      const doc = new HTMLParser(res.data.toString().trim())
      const ui = new HTMLParser(doc.getElementsByClassName('table')[0]['innerHTML'])
      ...
      for (let i = 1; i < trList.length; i++) {
        ...
        const bookname = spant1.attributes.title.replace(/\s/g, '')
        // 获取作者
        const spant2 = trdoc.getElementsByClassName('text-muted')[1].innerHTML.replace(/\s/g, '')
        const author = spant2
  ...
  search12: async keyword => {
    ...
    let url = 'https://www.sdhear.com/api/search?q=KEYword'.replace('KEYword', keyword)
    ...
    try {
      ...
      for (let i = 0; i < booksearch.length; i++) {
        ...
        book.push({
          ...
          bookname: booksc.title.replace(/<[^>]*>/g, ''),
          author: booksc.author.replace(/<[^>]*>/g, ''),
          origin: origin,
          tags: tags
        })
      }
  ...
  search13: async keyword => {
    ...
    let url = 'http://www.shuyaya.net/search/?searchkey=KEYword'.replace('KEYword', keyword)
    ...
    try {
      ...
      const doc = new HTMLParser(res.data.toString().trim())
      const ui = new HTMLParser(doc.getElementById('hotcontent').innerHTML)
      ...
      for (let i = 0; i < trList.length; i++) {
        ...
        // 获取封面
        const imgurl = trdoc.getElementsByClassName('lazy')[0].attributes['data-original']
        // 获取链接和作者
        const spant1 = trdoc.getElementsByTagName('a')[0]
        const bookurl = origin + spant1.attributes.href
        const bookname = spant1.attributes.title.replace(/\s/g, '')
        // 获取作者
        const spant2 = trdoc.getElementsByTagName('a')[2].innerHTML.replace(/\s/g, '')
        const author = spant2
  ...
  search14: async keyword => {
    ...
    let url = 'http://www.ppxs.net/search/?searchkey=KEYword'.replace('KEYword', keyword)
    ...
    try {
      ...
      const doc = new HTMLParser(res.data.toString().trim())
      const ui = new HTMLParser(doc.getElementById('main').innerHTML)
      const trList = ui.getElementsByTagName('li')
      let book = []
      for (let i = 1; i < trList.length; i++) {
        const trdoc = new HTMLParser(trList[i]['innerHTML'])
        ...
        // 分类 状态
        tags.push(trdoc.getElementsByClassName('s1')[0].innerHTML)
        tags.push(trdoc.getElementsByClassName('s6')[0].innerHTML)
  ...
  search15: async keyword => {
    ...
    let url = 'https://wechat.idejian.com/api/wechat/search/do?keyword=KEYword'.replace(
      'KEYword',
      keyword
    )
    // 浏览器端有跨域请求，需进行判断
    ...
    try {
      ...
      totalPage = firstResponse.data.body.pageInfo.totalPage
        ...
        promises.push(
          new Promise((resolve, reject) => {
            uni.request({
              ...
              success: res => {
                const search = res.data.body.books
                for (let i = 0; i < search.length; i++) {
                  const regex = /<b\s*style="color:red">|<\/b>/g
                  let imgurl = search[i].picUrl
                  // 加上书源 否则id可能和其它书源的url重复
                  let bookurl = origin + search[i].bookId
                  let bookname = search[i].bookName.replace(regex, '').replace(/\s/g, '')
                  let author = search[i].author.replace(regex, '').replace(/\s/g, '')
                  let intro = search[i].desc
                  let tags = []
                  if (search[i].tag) {
                    tags.push(search[i].tag)
                  }
                  book.push({
                    imgurl: imgurl,
                    bookurl: bookurl,
  ...
  search16: async keyword => {
    ...
    let url = 'http://api.jxgtzxc.com/search?keyword=KEYword'.replace('KEYword', keyword)
    // 浏览器端有跨域请求，需进行判断
    // #ifdef H5
    url = extractHostname(url)
    // #endif
