...
const getRead = {
  ...
  read1: async bookurl => {
    ...
    try {
      ...
      // 获取正文
      const intro = doc.getElementById('content')
      let int = intro.innerHTML.replace(/<br ?\/?>[\s　]*<br ?\/?>/g, '<br>').replace(/\s/g, '')
      console.log(int)
      let replacedStr = int
        .split('<br>')
        .filter(line => line.trim() !== '')
  ...
  read2: async bookurl => {
    ...
    try {
      ...
      while (true) {
        ...
        // 判断是否有下一页
        const next = new HTMLParser(
          doc.getElementsByClassName('section-opt')[0].innerHTML
  ...
  read5: async bookurl => {
    ...
    try {
      ...
      // 获取正文
      const intro = doc.getElementsByClassName('content')[0]
      let int = intro.innerHTML.replace(/&nbsp;/g, ' ').replace(/\s/g, '')
  ...
  read9: async bookurl => {
    ...
    try {
      ...
      while (true) {
        ...
        // 获取正文
        const intro = doc.getElementById('booktxt')
        let int = intro.innerHTML.replace(/\s/g, '').replace(/<br ?\/?>[\s　]*<br ?\/?>/g, '<br>')
        ...
        // 判断是否有下一页
        const next = new HTMLParser(
          doc.getElementsByClassName('readPager')[0].innerHTML
        ).getElementsByTagName('a')[2]
  ...
  read10: async bookurl => {
    ...
    try {
      ...
      while (true) {
        ...
        // 判断是否有下一页
        const next = new HTMLParser(
          doc.getElementsByClassName('u')[0].innerHTML
        ).getElementsByTagName('a')[2]
        url = next.attributes.href
        // #ifdef H5
        url = extractHostname(url)
  ...
  read11: async bookurl => {
    ...
    try {
      ...
      while (true) {
        ...
        // 判断是否有下一页
        const next = new HTMLParser(
          doc.getElementsByClassName('readpage')[0].innerHTML
  ...
  read12: async bookurl => {
    ...
    try {
      ...
      while (true) {
        ...
        // 判断是否有下一页
        const next = new HTMLParser(
          doc.getElementsByClassName('nr_page')[0].innerHTML
        ).getElementsByTagName('a')[2]
        url = parsedOrigin + next.attributes.href
        // #ifdef H5
        url = extractHostname(url)
  ...
  read13: async bookurl => {
    ...
    try {
      ...
      while (true) {
        ...
        // 判断是否有下一页
        const next = new HTMLParser(
          doc.getElementsByClassName('bottem1')[0].innerHTML
        ).getElementsByTagName('a')[2]
        url = parsedOrigin + next.attributes.href
        // #ifdef H5
        url = extractHostname(url)
  ...
  read15: async bookurl => {
    ...
    try {
      ...
      // 获取正文
      replacedStr += doc.getElementsByClassName('text-title-1')[0].innerHTML
